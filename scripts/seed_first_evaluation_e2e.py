#!/usr/bin/env python3
"""Create one disposable merchant AgentRank has never measured, for local browser tests.

The first-evaluation workflow needs less than the re-evaluation one and the difference is the
whole point: a registered benchmark world, a published suite authored against it, and one source
snapshot recording the merchant's own information. No compiler run, no published representation
and no benchmark run of any kind, because a merchant who has any of those is past the state this
workflow exists for.
...
A new merchant per invocation, because the workflow executes a real benchmark run and a second
pass over a merchant that already has one would no longer be a first evaluation.
...
# One purchasable mission and one control, for the same reason the re-evaluation seed uses two.
...
def _catalog(slug: str) -> dict[str, Any]:
    ...
    document["merchant_name"] = "First evaluation E2E"
    return document
...
def _suite(slug: str) -> dict[str, Any]:
    ...
    document["version"] = 1
    document["merchant_slug"] = slug
    document["name"] = "First evaluation E2E suite"
...
async def main() -> None:
    ...
    slug = f"first-e2e-{int(time.time())}"
    ...
    try:
        async with create_session_factory(engine)() as session:
            ...
            # The merchant's own information, and nothing derived from it. Compiling here would
            # put this merchant into the state the workflow under test is the way out of.
            ...
            issued = await MerchantCredentialService(session).issue(
                ...
                label="playwright first evaluation",
