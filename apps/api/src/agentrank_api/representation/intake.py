...
class MerchantSourceIntakeService:
    ...
    async def submit(
        ...
        *,
        ...
        if current is not None and _evidence(current.payload) == _evidence(definition.payload()):
            return await self._record(merchant, request_key, current, created=False)
        snapshot = await MerchantSourceRepository(self._session).create(merchant, definition)
        return await self._record(merchant, request_key, snapshot, created=True)
    ...
    async def current(self, merchant_id: uuid.UUID) -> MerchantSourceSnapshot | None:
        ...
        return (
            await self._session.execute(
                ...
                .order_by(
                    MerchantSourceSnapshot.created_at.desc(), MerchantSourceSnapshot.id.desc()
                )
                .limit(1)
            )
        ).scalar_one_or_none()
    ...
    async def overview(self, merchant_id: uuid.UUID, *, limit: int = 20) -> SourceOverviewView:
        ...
        rows = list(
            (
                await self._session.execute(
                    ...
                    .where(MerchantSourceSnapshot.merchant_id == merchant_id)
                    .order_by(
                        MerchantSourceSnapshot.created_at.desc(),
                        MerchantSourceSnapshot.id.desc(),
                    )
                    .limit(limit)
                )
            ).all()
        ...
        return SourceOverviewView(
            current_source_snapshot_id=None if current is None else current.id,
            snapshots=summaries,
        )
    ...
    async def submission_view(self, outcome: SubmissionOutcome) -> SourceSubmissionView:
        ...
        current = await self.current(outcome.submission.merchant_id)
        row = (
            await self._session.execute(
    ...
    async def _record(
        ...
        *,
        ...
        try:
            ...
            duplicate = await self._by_request_key(merchant.id, request_key)
