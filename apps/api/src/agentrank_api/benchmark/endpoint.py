...
run, an oracle or a mission definition. The merchant evaluation command is the one benchmark
surface that is an endpoint, and it is built out of this application rather than left mounted:
a buyer that could queue an evaluation could touch the lifecycle of the run it is inside.
...
class LocalCommerceEndpoint:
    ...
    def __init__(
        ...
        *,
        ...
        # buyer is given a credential for, and a buyer that could queue an evaluation could
