...
describe("preflight decoding", () => {
  ...
  it("carries every blocker with its own sentence", () => {
    ...
    expect(preflight.blockers[0]?.code).toBe("reevaluation_already_pending");
    expect(preflight.pending_reevaluation_id).not.toBeNull();
  });
...
describe("launch decoding", () => {
  it("keeps a queued launch free of any run fact", () => {
    const launch = decodeReevaluation(QUEUED_REEVALUATION_FIXTURE);
  ...
  it("decodes a list", () => {
    expect(decodeReevaluationList([QUEUED_REEVALUATION_FIXTURE])).toHaveLength(1);
  });

  it("refuses an unknown status rather than passing it through", () => {
    expect(() => decodeReevaluation({ ...QUEUED_REEVALUATION_FIXTURE, status: "DONE" })).toThrow(
      DecodeError,
    );
  });

  it("decodes a comparison with its caveats and per currency demand", () => {
    const detail = decodeReevaluationDetail(COMPLETED_REEVALUATION_FIXTURE);
  ...
  it("keeps an absent comparison null rather than an empty one", () => {
    expect(decodeReevaluationDetail(QUEUED_REEVALUATION_FIXTURE).comparison).toBeNull();
  });
});
