...
def executor(session: AsyncSession, merchant_id: uuid.UUID) -> ReferenceMissionExecutor:
...
async def test_the_order_missions_run_in_does_not_change_what_they_mean(
    ...
    forwards = await run(session, merchant_id)
    backwards = await run(session, merchant_id, version=2)
...
async def test_a_second_run_does_not_inherit_the_first_ones_stock(
    ...
    first = await run(session, merchant_id)
    second = await run(session, merchant_id)
...
async def test_a_run_records_which_executor_produced_it(session: AsyncSession) -> None:
...
async def test_a_run_records_every_pin_it_has(session: AsyncSession) -> None:
...
async def test_a_mission_is_marked_against_the_catalog_it_was_given(
    ...
    finished = await run(session, merchant_id, fixture=scarce)
